...
class LoginForm(FlaskForm):
    username = StringField('username', validators=[DataRequired(message = u'用户名不能为空'),Length(1,64),
        Regexp('^[A-Za-z][A-Za-z0-9_.]*$',0,u'用户名只能由字母，数字，下划线和.构成，且必须以字母开始')])
    password = PasswordField('password',validators = [DataRequired(message = u'密码不能为空'),Length(1,120)])
...
class SignupForm(FlaskForm):
    username = StringField('username', validators=[DataRequired(message = u'用户名不能为空'),Length(1,64),
        Regexp('^[A-Za-z][A-Za-z0-9_.]*$',0,u'用户名只能由字母，数字，下划线和.构成，且必须以字母开始')])
    password = PasswordField('password',validators = [DataRequired(message = u'密码不能为空'),Length(1,120)])
    repassword = PasswordField('repassword',validators = [DataRequired(message = u'密码不能为空'),Length(1,120),EqualTo('password',message = u'两次输入不同')])
    email = StringField('email',validators = [DataRequired(message = u'邮箱不能为空'),Length(1,64),Email()])
